...
export class TmtpSidebarProvider implements vscode.WebviewViewProvider {
  ...
  constructor(
    ...
    private readonly choosePracticeFile: () => void,
  ...
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    view.webview.onDidReceiveMessage((message: { type?: string; tab?: WorkspaceTab }) => {
      ...
      if (message.type === 'practice') this.choosePracticeFile();
    });
    this.render();
  }
    ...
    webview.html = `<!doctype html><html><head>
      ...
      <style nonce="${nonce}">
        *{box-sizing:border-box}body{margin:0;padding:14px;color:var(--vscode-foreground);font:var(--vscode-font-size) var(--vscode-font-family)}
        ...
        button{width:100%;text-align:left;border:0;border-radius:4px;padding:8px 9px;margin-top:6px;color:var(--vscode-button-secondaryForeground);background:var(--vscode-button-secondaryBackground);font:inherit;cursor:pointer}button:hover{background:var(--vscode-button-secondaryHoverBackground)}button:disabled{opacity:.45;cursor:not-allowed}
      ...
      <button class="primary" data-action="practice" ${s.startingFileCount === 0 ? 'disabled' : ''}>Practice this File</button>
