...
You are given "files" — the title and project responsibility already established for each toured file — and "scenarios": an ordered, fixed list of scenario slots. Each slot already has a "file" (the correct answer), an "options" list (always including "file"), and a "difficulty" tier. A slot may also have "learningFile": the file the developer deliberately chose to practise. In that case, keep the situation centered on understanding that learning file's responsibility and architectural boundaries, but respect "file" as the correct answer even when it is a related file. You do not choose these fields — you only write the situation and explanation for each slot, in order.
...
- For focused practice with "learningFile", do not make that file the obvious answer by repeatedly asking what it owns. Test both sides of its boundary: what belongs in it, what should call it, what it delegates elsewhere, and which neighboring file should change instead. Never imply that "learningFile" must be the answer.
